...
public class Percolation {
	...
	private static final int[][] d = { { 0, -1 }, { -1, 0 }, { 0, 1 }, { 1, 0 } }; // dx, dy
	private static final int Neighbours = 4; // number of sites that are next to each site
    ...
    */
	...
	private void validate(int i, int j) {
		if (i < 1 || i > N) throw new IndexOutOfBoundsException("i index " + i + " is not in range 1 - " + N);
		if (j < 1 || j > N) throw new IndexOutOfBoundsException("j index " + j + " is not in range 1 - " + N);
	}
	...
	public void open(int i, int j) {
		if (!isOpen(i, j)) {
			...
			for (int k = 0; k < Neighbours; k++) {
				if (isInside(i + d[k][0], j + d[k][1]) &&
					isOpen(i + d[k][0], j + d[k][1])) {
					uf.union(currSiteIndex, getIndex(i - 1 + d[k][0], j - 1 + d[k][1]));
				}
			}
		}
	}
	 ...
	 * @param i row
	 * @param j column
	 * @return <tt>true</tt> if the site <tt>(i, j)</tt> is open, and <tt>false</tt> otherwise
	 * @throws java.lang.IndexOutOfBoundsException unless 1 <= i, j <= N
	 */
	 ...
	 * @return <tt>true</tt>, if the system percolates, and <tt>false</tt> otherwise
	 */
